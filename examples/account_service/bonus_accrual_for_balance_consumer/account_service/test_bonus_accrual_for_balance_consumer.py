...
pytestmark = [
    ...
    allure.description(
        'Асинхронная обработка Portfolio.Bonus.Operation (bonusAccrualForBalance) в AccountService: '
...
class TestPositive:
    pytestmark = allure.story('Positive. Обработка Portfolio.Bonus.Operation (bonusAccrualForBalance)')
    ...
    def _before(self):
        ...
        with allure.step('Добавляем клиента с бонусным счётом в БД AccountService'):
            self.client_from_db_before = self.db.add_bonus_client(self.client)
    ...
    def _send_message(self):
        with allure.step('Генерируем сообщение'):
            topic = PortfolioBonusOperationTopic(
                client=self.client,
                event_type='bonusAccrualForBalance',
                amount=self.amount,
            )
    ...
    @allure.title('Проводка начисления в БД AccountService')
    def test_bonus_accrual_transaction_created(self):
        ...
        with allure.step('Проверяем проводку начисления бонусов'):
            ...
            check_assert_that(transaction.TypeId, equal_to(1), 'Ошибка в TypeId')  # Credit — начисление
            check_assert_that(transaction.EventId, equal_to(7771), 'Ошибка в EventId')  # BonusAccrualForBalance
            check_assert_that(transaction.StatusId, equal_to(1), 'Ошибка в StatusId')  # Completed
            check_assert_that(transaction.Description, equal_to('Начисление бонусов'),
                              'Ошибка в Description')

    @allure.title('Начисление бонусов во FrozenBalance')
    def test_frozen_balance_increased(self):
        with allure.step('Проверяем баланс после начисления'):
            balance_after = self.client_from_db_after.one_account.balance
            allure_attach(self.balance_before, 'balance_before')
            allure_attach(balance_after, 'balance_after')
            check_assert_that(balance_after.ActiveBalance, equal_to(self.balance_before.ActiveBalance),
                              'ActiveBalance не должен меняться при начислении')
            check_assert_that(balance_after.FrozenBalance,
                              equal_to(self.balance_before.FrozenBalance + self.amount),
                              'FrozenBalance должен увеличиться на сумму начисления')
            check_assert_that(
                balance_after.is_equal(self.balance_before, ignored=['FrozenBalance', 'ModifiedOn']),
