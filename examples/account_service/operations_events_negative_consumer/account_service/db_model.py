...
from sqlalchemy import BigInteger, Boolean, CHAR, DateTime, ForeignKeyConstraint, Identity, Index, Integer, LargeBinary, Numeric, PrimaryKeyConstraint, SmallInteger, String, Text, UUID, text, Uuid
...
class Account(Base):
    ...
    balance: Mapped[Optional['Balance']] = relationship(
        'Balance', back_populates='account', uselist=False, lazy='joined', viewonly=True
    )
    balance_transactions: Mapped[List['BalanceTransaction']] = relationship(
        'BalanceTransaction', back_populates='account',
        order_by='BalanceTransaction.Id', lazy='joined', viewonly=True
    )
    ...
    def set_bonus(self, client: data_generator.Client):
        """Бонусный счёт (TypeId=3, StatusId=20, CurrencyId=9991)"""
        self.CreatedOn = client.one_account.created_on
        self.Id = client.one_account.id
        self.ClientId = client.id
        self.TypeId = 3  # Бонусный
        self.StatusId = 20  # Активный
...
class Client(Base):
    ...
    def set_bonus(self, client: data_generator.Client):
        """Создание клиента с бонусным счётом (LevelId=10, Account TypeId=3, CurrencyId=9991)"""
        self.set_anonymous_with_client_type(client)
        self.LevelId = 10  # Идентифицированный
        account = Account()
        account.set_bonus(client)
        self.accounts[0] = account
...
class Balance(Base):
    ...
    def set_values(self, client_id, account_id, active_balance, frozen_balance=0):
        now = datetime_now()
        self.AccountId = account_id
        self.ClientId = client_id
        self.ActiveBalance = active_balance
...
class BalanceTransaction(Base):
    ...
    ClientId: Mapped[str] = mapped_column(UUID(as_uuid=False))
    AccountId: Mapped[str] = mapped_column(UUID(as_uuid=False))
    OperationId: Mapped[Optional[int]] = mapped_column(BigInteger)
    ...
    account: Mapped['Account'] = relationship('Account', back_populates='balance_transactions')

    def set_values(self, client_id, account_id, operation_id, amount, currency_id,
                   type_id, event_id, description, status_id=1):
        now = datetime_now()
        self.ClientId = client_id
        self.AccountId = account_id
